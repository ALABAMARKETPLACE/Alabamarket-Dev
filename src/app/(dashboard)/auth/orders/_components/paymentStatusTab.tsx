    ...
    >
      ...
      <div className="d-flex flex-column gap-2">
        <div className="d-flex justify-content-between border-bottom pb-2">
          ...
          <span className="fw-medium text-capitalize">{props?.data?.orderPayment?.paymentType ?? "N/A"}</span>
        </div>
        
        <div className="d-flex justify-content-between">
          <span className="text-muted">Product Total:</span>
          <span>{settings.currency === "NGN" ? "₦" : settings.currency} {formatCurrency(props?.data?.total)}</span>
        </div>
        
        <div className="d-flex justify-content-between">
          <span className="text-muted">Tax:</span>
          <span>{settings.currency === "NGN" ? "₦" : settings.currency} {formatCurrency(props?.data?.tax)}</span>
        </div>
        
        <div className="d-flex justify-content-between">
          <span className="text-muted">Discount:</span>
          <span className="text-success">-{settings.currency === "NGN" ? "₦" : settings.currency} {formatCurrency(props?.data?.discount)}</span>
        </div>
        ...
        <div className="bg-light p-3 rounded mt-3">
          ...
          <div className="d-flex flex-column gap-2">
            <div className="d-flex justify-content-between">
              ...
              <span className={`badge ${props?.data?.orderPayment?.status === 'success' ? 'bg-success' : 'bg-warning'} text-white fw-normal text-uppercase`}>
            ...
            {props?.data?.orderPayment?.ref && (
              <div className="d-flex justify-content-between">
                ...
                <span className="font-monospace small">{props?.data?.orderPayment?.ref}</span>
              </div>
            )}
            
            <div className="d-flex justify-content-between">
              <span className="text-muted">Date:</span>
              <span>{moment(props?.data?.orderPayment?.createdAt).format(DATE_FORMAT)}</span>
            </div>
            
            <div className="d-flex justify-content-between">
              <span className="text-muted">Order ID:</span>
              <span className="font-monospace small">{props?.data?.order_id}</span>
            </div>
          </div>
        </div>
      </div>
