...
function Page() {
  ...
  const longitude =
    Settings?.isLocation === true && Location.longitude
      ? Location.longitude
      : 0;
  ...
  const getProducts = async (pageNumber: number, currentType: string) => {
    ...
    if (isFeatured) {
      ...
      const position = positionParam === "discounted" ? "4" : positionParam;
      url = `${API.FEATURED_POSITION_PRODUCTS}/${position}/products?page=${pageNumber}&take=${featuredPageSize}`;
