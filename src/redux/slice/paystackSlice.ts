  ...
  reducers: (create) => ({
    ...
    verifyPayment: create.asyncThunk(
      async (reference: string, { rejectWithValue }) => {
        try {
          const response = await POST(API.PAYSTACK_VERIFY, {
            reference,
    ...
    processRefund: create.asyncThunk(
      async (
        refundData: { transaction: string; amount?: number; reason?: string },
        { rejectWithValue },
      ) => {
        try {
          const response = await POST(
            API.PAYSTACK_REFUND,
            refundData as unknown as Record<string, unknown>,
          );
          return response;
        } catch (err: unknown) {
          const error = err as ApiError;
    ...
    // Synchronous actions
    setPaymentReference: create.reducer(
      (state, action: PayloadAction<string | null>) => {
        state.paymentReference = action.payload;
      },
    ),

    setPaymentStatus: create.reducer(
      (state, action: PayloadAction<PaystackState["paymentStatus"]>) => {
    ...
    setVerificationData: create.reducer(
      (state, action: PayloadAction<PaystackVerificationData | null>) => {
...
// Selectors
export const selectPaystackState = (state: { paystack: PaystackState }) =>
  state.paystack;
export const selectIsLoading = (state: { paystack: PaystackState }) =>
  state.paystack.isInitializing ||
  state.paystack.isVerifying ||
  state.paystack.isRefunding;
export const selectPaymentStatus = (state: { paystack: PaystackState }) =>
  state.paystack.paymentStatus;
export const selectPaymentReference = (state: { paystack: PaystackState }) =>
  state.paystack.paymentReference;
export const selectVerificationData = (state: { paystack: PaystackState }) =>
  state.paystack.verificationData;
export const selectPublicKey = (state: { paystack: PaystackState }) =>
  state.paystack.publicKey;
